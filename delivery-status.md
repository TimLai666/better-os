...
| M50 | v0.2.3 public release — the field-reported UI fixes (needs M49) | agent | done | workspace version 0.2.2 → 0.2.3 across 52 crates, the seven shipped manifests re-pointed at the 0.2.3 assets with checksums back to placeholders per the ADR 0002 rule, and `docs/third-party-licenses.md` regenerated because it pins the `Cargo.lock` hash; no test hard-codes the version and `install.sh` reads it from the public API, so the bump touched no code. One release-tooling defect was found and fixed rather than worked around: `packaging/build-deb.sh` never cleared `dist/`, and `verify-deb.sh` picks each package with a version-agnostic glob that fails when it matches more than one file, so the eight stale `_0.2.0_` sidecars and eight `_0.2.2_` packages sitting there would have failed the verifier with a message naming the package instead of the leftover — the build script now clears the top-level `.deb` and `.deb.sha256` before building, leaving the scratch subdirectories other tools own alone, and the run after it produced exactly 8 and 8. Full gate green locally before the merge (fmt, check, clippy `-D warnings`, and `build-deb.sh` plus `verify-deb.sh` end to end on amd64 over all eight packages); post-merge CI run [33955547574](https://github.com/TimLai666/better-os/actions/runs/33955547574) on merge commit `056eaad` green across all six jobs on its first attempt — rust, installer, and the four package jobs, with no red run and no retry. [`v0.2.3`](https://github.com/TimLai666/better-os/releases/tag/v0.2.3) published with 66 assets — 32 `.deb`, 32 `.deb.sha256`, `LICENSE`, and `third-party-licenses.md` — the full 8 × 2 × 2 matrix enumerated component by component with no filename collisions. Every asset was then downloaded again over plain HTTPS from the public release URL, all 32 sidecars verified, and the re-downloaded `.deb` bytes compared byte-for-byte against the CI artifacts. The 28 artifact variants across the seven manifests now carry the published 0.2.3 checksums, each re-hashed from the downloaded package rather than copied from a sidecar, and then cross-checked a second time against the sidecars by a separate script — so a mistake in the writer could not pass by repeating itself. 167 test targets and 2,506 tests pass against the real checksums. `bash install.sh --dry-run` resolved `v0.2.3` from the public API and named the four 24.04 amd64 URLs it would fetch, reporting the 0.2.2 currently on this host as the version it would replace. Two things this release did not do, both carried forward from M49: nobody pressed Escape on Better Launcher, and nobody has watched GNOME draw the six new icons after an install — `verify-deb.sh` asserts each entry's `Icon=` names a file its own package carries, which is a packaging check and not a rendering one |
| M51 | v0.2.4 public release — the Zorin daemon refusal, the invisible icons, the hidden refusal reason, and the launch panic (needs M50) | agent | done | ticket 43 was cut from a second round of field reports on the same real Zorin OS 18 install, this time of `v0.2.3`, and it fixed four defects at their own causes: `manager-daemon` read only `VERSION_ID` from `/etc/os-release`, so Zorin 18 looked like release 18 and *every* plan was refused with `plan targets release 24.04 but this host is 18` — the same defect ticket 40 fixed in `install.sh`, present in the second place that reads os-release, and now resolving the Ubuntu base from `UBUNTU_CODENAME` with five parser tests including the verbatim Zorin 18.1 shape; the six icons v0.2.3 shipped were invisible because gdk-pixbuf sniffs a file's first bytes to choose a loader and the attribution comment between the XML declaration and `<svg>` pushed the root tag past that window, reproduced with `gdk-pixbuf-thumbnailer` and bisected to the comment, with `verify-deb.sh` gaining a sniff-window assertion so it cannot ship again; the failure card collapsed every `daemon.*` evidence key into one localized sentence and dropped the machine detail, which is why diagnosing the refusal above took a state-file dig, and now shows a Technical detail row carrying the service's own untranslated words; and the `there is no reactor running` panic on every window launch, pre-existing and recorded at ticket 42, is gone because `manager-platform`'s D-Bus client keeps one process-wide tokio reactor and enters it before connections and proxies — workspace feature unification builds zbus with its `tokio` flavor, which spawns internal tasks onto an ambient runtime even behind the blocking API. Workspace version 0.2.3 → 0.2.4, the seven shipped manifests re-pointed at the 0.2.4 assets with checksums back to placeholders per the ADR 0002 rule, and `docs/third-party-licenses.md` regenerated; no test hard-codes the version and `install.sh` reads it from the public API, so the bump touched no code. One process failure is recorded rather than hidden: the ticket-43 merge commit `c813fd6` went **red** in CI run [33970589800](https://github.com/TimLai666/better-os/actions/runs/33970589800) because it moved `Cargo.lock` — the new optional `tokio` dependency — without regenerating the license inventory that pins the lockfile hash, which is exactly the rule `AGENTS.md` states; the bump commit regenerated it and the condition was repaired by the next commit, but the red run stays in `main`'s history. Full gate green locally before the merge (fmt, check, clippy `-D warnings`, 167 test targets and 2,510 tests, and `build-deb.sh` plus `verify-deb.sh` end to end on amd64 over all eight packages, leaving exactly 8 `.deb` and 8 `.deb.sha256` in `dist/`); post-merge CI run [33970875135](https://github.com/TimLai666/better-os/actions/runs/33970875135) on merge commit `42be13d` green across all six jobs on its first attempt. [`v0.2.4`](https://github.com/TimLai666/better-os/releases/tag/v0.2.4) published with 66 assets — 32 `.deb`, 32 `.deb.sha256`, `LICENSE`, and `third-party-licenses.md` — the full 8 × 2 × 2 matrix enumerated component by component with no filename collisions and nothing unexpected. Every package was then fetched again with plain `curl` from the exact public `releases/download/v0.2.4/` URL each manifest carries, so the URLs are proven to resolve and not merely assumed; all 32 sidecars verified and all 32 downloaded packages compared byte-for-byte against the CI artifacts with no difference. The 28 artifact variants across the seven manifests now carry the published 0.2.4 checksums, each re-hashed from the downloaded package rather than copied from a sidecar, then cross-checked a second time against the sidecars by a separate reader so a mistake in the writer could not pass by repeating itself. 167 test targets and 2,510 tests pass against the real checksums. `bash install.sh --dry-run` resolved `v0.2.4` from the public API. What this release still does not have is a pair of human eyes on a desktop: nobody pressed Escape on Better Launcher, and although the icon defect was reproduced and fixed against the host's own gdk-pixbuf, nobody has watched GNOME draw the six icons in an applications grid after an install |

| M52 | ticket 45 — the unprivileged client reads the real host (needs M51) | agent | done | `manager-gui` and `manager-cli` built their `Manager` from `MockPlatform::default()`, which reports `ubuntu 24.04 amd64` on every machine, and `Manager::artifact_for_profile` picks an artifact by exactly that release and architecture — so on a 22.04 or arm64 host every plan named a package the host cannot use. It looked correct on the project's Zorin 18 machine only because ticket 43 taught the daemon to resolve that host to 24.04 as well, so the assumption and the real answer happened to agree; the checksums cannot match across releases, so nothing wrong could be installed, but the failure would have arrived from the daemon as a mismatch message rather than from the client as a host it never asked about. The fix did not add a third copy of the os-release rules: `install.sh` (ticket 40) and `manager-daemon/src/host.rs` (ticket 43) each had one, and a third is how a client and the service that checks its plan drift apart. `crates/better-core/src/host.rs` now owns them — both Rust readers call it, `install.sh` is shell and keeps a matching copy with a comment pointing at it, and the daemon's five parser tests were kept unchanged as the proof its behavior survived. The shared table is `install.sh`'s, which was the stricter of the two: `VERSION_CODENAME` counts only for `ID=ubuntu`, the `VERSION_ID` fallback is Ubuntu's alone, and it is accepted only when it names a release in the matrix — each case the daemon used to resolve and now refuses was one it would have refused a step later with a worse message. `manager-platform`'s new `HostPlatform` reads the release from `/etc/os-release`, the architecture from `dpkg --print-architecture` (both read-only, so both unprivileged), and the distribution from `ID`, which is now the real `zorin` rather than an assumed `ubuntu` — every first-party manifest declares `[zorin, ubuntu]` so nothing first-party changes, and `defaults-core` already treated `zorin` as its own value. An unsupported host is a new `PlatformError::UnsupportedHost` carrying a one-line description of the fields that were read, and never a fallback profile: the CLI stops, and the GUI draws an explicit unsupported-host banner in both locales over a `unknown`/`unknown`/`unknown` profile no manifest declares, so every plan fails with "this component ships nothing for this host" instead of quietly aiming at 24.04. Demo mode keeps `MockPlatform`, which is the only thing it was ever for. 25 new tests, all fixture-driven and none reading the machine they run on: 13 in `better-core` (Zorin 17 and 18, plain Ubuntu with and without a codename, an unknown codename, an out-of-matrix Ubuntu, Fedora, the `ID`/`ID_LIKE` prefix trap), 6 in `manager-platform`, 4 in `manager-gui`, and 2 in `manager-cli`, with each client's mode-to-platform mapping asserted through a `ClientPlatform` seam rather than by probing the test host. Full gate green: fmt, check, clippy `-D warnings`, 167 test targets and 2,555 tests, an 8-second `ZED_HEADLESS=1` smoke on a scratch XDG with `BETTER_MANAGER_OFFLINE=1`, and a real-mode on-host run whose sidebar footer was captured reading `zorin 24.04` over `amd64` — the first time it has shown what was read rather than what was assumed. Two limits travel with it: no 22.04 and no arm64 machine was available, so those hosts are proved by fixture and not by a run, and an architecture outside the matrix still produces a per-component "no artifact" rather than the one statement about the machine that `install.sh` gives |
...
| M57 | ticket 49 — drift self-heal, an action for the rest, and the CLI panic that hung (needs M56) | agent | done | the fifth round of field reports from the same Zorin 18 machine, on `v0.2.5`, and three unrelated causes behind them. **The manager punished a user for upgrading it the only way it can be upgraded.** The state record said `better-manager 0.2.3`, health failed, the old `plan_rejected` failure, provenance manager, drift `version_mismatch` against a host at 0.2.5; dpkg held 0.2.5 and the catalog 0.2.6, and the Updates screen offered nothing, because drift blocks planning and reconciliation called an external `install.sh` upgrade drift. Reconciliation now decides every case in one closed set, `HostComparison`: a host *ahead* of the record self-heals — `installed_version` and `provenance` adopt dpkg's answer, the stale failure and its failed health are cleared because they describe a version that is no longer installed, the artifact and the restore snapshot go with them because they name a version this machine no longer has, `enabled` is kept because dpkg has no opinion about it, and an activity entry records `host.externally_upgraded:0.2.3->0.2.5` — while every dangerous direction stays blocking: a host behind the record, a package the record claims and dpkg has never heard of, and a version `semver` cannot order on either side. Proved against the machine's real record, copied to a scratch path rather than mutated: `reconcile` healed it to 0.2.5 healthy dpkg with no finding, and `plan better-manager update` then produced the 0.2.5 → 0.2.6 the screen had been refusing to offer. The remaining blocking half was invisible in the window — drift appeared on no screen at all, so the planner refused everything and nothing said why. The component page now draws a drift card with what disagrees and what it costs, and a 採用系統版本 action that asks before it acts, because adoption throws away the recorded restore point; the action is `Manager::adopt_host_state` in `manager-core`, not state-file surgery in the GUI, so the window and `better-manager-cli reconcile --adopt <id>` obey the same rules, and it refuses an unknown component, a running transaction, and a version nothing can order, and writes nothing when the record already agrees. **`/usr/bin/better-manager catalog status` hung because `/usr/bin/better-manager` is the window**: `build-deb.sh` installed `manager-gui` alone and the command line was never packaged, and the window ignored every argument and opened a window — a program that does not exit is what a hang looks like from a terminal. The package now installs `manager-cli` as `/usr/bin/better-manager-cli`, following `better-monitor`'s existing split, and the window refuses an argument with exit 2 naming the program that answers it, while answering `--version` and `--help` where they are asked. **The panic was not ours to guard.** The backtrace names `gpui_linux::xdg_desktop_portal::XDPEventSource::new` on a gpui worker thread and `accesskit_unix::context::get_or_init_messages` on its own: zbus picks its I/O backend from a compile-time cargo feature, cargo unifies features across every package one `cargo build` names, and `build-deb.sh` built the services and the windows together — so six service crates asking for `zbus/tokio` compiled that flavor into every Better OS **window**, where two third-party libraries open their own connections from plain threads no tokio runtime owns. Ticket 43's `runtime().enter()` could only ever cover this project's own calls. The feature is gone from all ten manifests and `zbus_polkit` with it; `manager-platform/src/flavor.rs` scans every workspace manifest and fails with the file and line if it comes back. The service side paid for it and was fixed at its own cause: the tokio flavor had been making zbus poll served method bodies as tokio tasks, so `manager-daemon` and `storage-service` could call `tokio::spawn` inside a handler — four of the daemon's six D-Bus tests hung on that until both captured a `tokio::runtime::Handle` at construction and spawned through it. 25 new tests and 2 rewritten (2,624 passing workspace-wide, 0 failing) — 9 in `manager-core`, 5 in `manager-gui`'s view models, 3 in `manager-platform::flavor`, 5 in `manager-cli` and 3 in `manager-gui` running the built binaries, including the reconciliation matrix as one table over host newer/equal/older/missing plus an epoch and an unorderable version, against both provenances, against a clean record and one carrying a real failed install; and regression tests that run the built binaries — the CLI's `catalog status`, `status <id>`, `status`, and `reconcile` under a 30-second timeout asserting no panic and no hang, and the window's refusal of a command line asserting exit 2 without opening anything. Full gate green: fmt, `cargo check --workspace --all-targets`, clippy `-D warnings`, 122 test targets and 2,624 tests. On this host, from the built binaries: `catalog status` and `status better-manager` both exited 0 with empty stderr, and the window — which printed two zbus reactor panics on the real Wayland session before the change — now runs silently both headless and on the session. Two limits: `build-deb.sh` was syntax-checked but not run, because it clears `dist/` of the published 0.2.6 packages; and `better-monitor`, `better-files`, `better-launcher`, `better-touchpad` and `better-awake` still ignore command-line arguments and open a window, which is named in the ticket rather than fixed, since only the manager has a shipped command line to point at |
| M58 | v0.2.7 public release — drift self-heal, the packaged command line, and the end of the window panics (needs M57) | agent | done | the release ticket 49 was waiting for, and the one the reporting machine could not work around: on `v0.2.6` a manager upgraded through `install.sh` recorded the result as drift, drift blocks planning, and the Updates screen therefore offered nothing for the one component that had an update. Workspace version 0.2.6 → 0.2.7, the seven shipped manifests re-pointed at the 0.2.7 assets with checksums back to placeholders per the ADR 0002 rule, and `docs/third-party-licenses.md` regenerated because it pins the `Cargo.lock` hash the version bump moved. No version hard-code needed fixing, and that was checked rather than assumed: a repository-wide search for a literal `0.2.6` outside the manifests and `Cargo.lock` found nothing at all. Full gate green locally before the merge: fmt, `cargo check --workspace`, clippy `-D warnings`, 170 test targets and 2,624 tests, `packaging/generate-third-party-notices.sh --check`, and `build-deb.sh` plus `verify-deb.sh` end to end on amd64 over all eight packages, leaving exactly 8 `.deb` and 8 `.deb.sha256` in `dist/` — this is also where the packaged command line was confirmed as a payload fact rather than a build-script reading, since `better-manager`'s `.deb` carries both `/usr/bin/better-manager` and `/usr/bin/better-manager-cli`. Post-merge CI run [34039904348](https://github.com/TimLai666/better-os/actions/runs/34039904348) on merge commit `adb3f44` green across all six jobs on its first attempt — rust, installer, and the four package jobs — with no red run and no retry. [`v0.2.7`](https://github.com/TimLai666/better-os/releases/tag/v0.2.7) published with 66 assets — 32 `.deb`, 32 `.deb.sha256`, `LICENSE`, and `third-party-licenses.md` — the full 8 × 2 × 2 matrix enumerated component by component with nothing missing and nothing unexpected. Every package was then fetched again with plain `curl` from the exact public `releases/download/v0.2.7/` URL each manifest carries, so the 28 URLs are proven to resolve rather than assumed; all 32 sidecars verified, and all 32 downloaded packages compared byte-for-byte against the CI artifacts with no difference. The 28 artifact variants across the seven manifests now carry the published 0.2.7 checksums, each re-hashed from the downloaded package rather than copied from a sidecar, then cross-checked a second time against those sidecars by a separate reader so a mistake in the writer could not pass by repeating itself. 170 test targets and 2,624 tests pass against the real checksums. `bash install.sh --dry-run` resolved `v0.2.7` from the public API, named the four 24.04 amd64 URLs it would fetch, and reported the 0.2.5 on this host as the version it would replace; nothing was installed. What this release does not prove is the field report closing. The self-heal was proved against the reporting machine's real state record copied to a scratch path, and the window's silence was seen on this host's own Wayland session, but nobody has yet watched that Zorin 18 machine upgrade to 0.2.7 and find its update waiting — and the same machine's Better Awake install on 0.2.6 is still unobserved. The container end-to-end check remains CI-only from every worktree here, for want of a Docker daemon |
...
The current release is `v0.2.7`. CI run 34039904348 on merge commit `adb3f44`
...
`releases/download/v0.2.7/` URL its own manifest carries, all 32 sidecars
...
Ticket 49 is three field reports from one Zorin 18 machine with three unrelated
causes. Reconciliation self-heals the safe direction: a host that dpkg says is
*ahead* of the manager's record is the machine having been upgraded through
`apt` or `install.sh`, which is the only way the manager can be upgraded, so the
record adopts dpkg's version and clears the stale failure, the failed health,
and the artifact and restore snapshot that name a version the machine no longer
has. Every other disagreement stays blocking, and a component in that state now
has both a screen and an action — a drift card saying what disagrees and what
adoption costs, and `Manager::adopt_host_state` behind it, reached identically
from the window and from `better-manager-cli reconcile --adopt <id>`. The
command line is packaged as `/usr/bin/better-manager-cli`, and
`/usr/bin/better-manager` refuses an argument it does not understand with exit 2
naming it rather than opening a window a terminal cannot escape. And zbus is
back on its default `async-io` flavor across the workspace, which ends the "there
is no reactor running" panic in every Better OS window: the backend is a
compile-time cargo feature, cargo unifies features across a single `cargo build`,
and `build-deb.sh` builds the services and the windows together, so six service
crates asking for `zbus/tokio` compiled that flavor into gpui's portal thread and
accesskit's, neither of which any guard in this project could reach.

`v0.2.6` before it published the same eight packages from CI run 34019971299 on
merge commit `3c8f2a0`. It carries tickets 46, 47, and 48; ticket 47 is the one
a machine could not work around, replacing a health check that derived
`/usr/bin/<component>` from the package name — wrong for three of the eight
shipped packages — with dpkg's own file list, read against dpkg's own
`--path-exclude` configuration.
...
reconciliation self-heals a host that is ahead of the record instead of calling
an external upgrade drift, a component whose drift still blocks planning says so
on its own page and offers 採用系統版本 through `Manager::adopt_host_state`, and
the two halves of the reported "CLI panic and hang" are fixed at their causes —
the package now ships `better-manager-cli` and the window refuses a command line
instead of opening a window, and zbus's tokio flavor is gone from the workspace
so gpui's portal and accesskit stop panicking in every Better OS window.
`docs/tickets/49-drift-self-heal-and-cli-hang.md` has the reasoning. What it
owes is an observation: the self-heal was proved against the reporting machine's
real state record copied to a scratch path, not on that machine, and nobody has
watched it upgrade to 0.2.7 and find the update it had been refused. Five
windows — Better Monitor, Better Files, Better Launcher, Better Touchpad, and
Better Awake — still swallow their arguments and open, because none of them has
a shipped command line to point a refusal at.
...
檢查更新 button that runs the existing catalog refresh off the UI thread and
then says what it found, or which list it is showing instead when the fetch did
not land. The reasoning, including why the titlebar was left alone and why a
...
rules for reading it live in one place — `crates/better-core/src/host.rs`,
called by both the daemon and the new `HostPlatform`, with `install.sh` keeping
the matching shell copy it cannot share. The sidebar footer shows what was read
rather than what was assumed: `zorin 24.04` over `amd64` on the machine it was
written on. An unsupported host is an explicit refusal at both clients and never
a fallback profile. The reasoning is in
`docs/tickets/45-real-client-platform-probe.md`.

The shape worth carrying out of it: the same rule written twice will be written
a third time, and the third copy is where the drift starts. Ticket 43 fixed the
...
install because it read `VERSION_ID` instead of resolving the Ubuntu base from
`UBUNTU_CODENAME`, the six icons 0.2.3 had just shipped were invisible because
the attribution comment pushed the SVG root past gdk-pixbuf's sniff window, the
refusal on screen dropped the machine detail the service actually gave, and
every window launch panicked with `there is no reactor running`. The record of
what each class is lives in `docs/tickets/43-zorin-daemon-and-icon-fixes.md`.

The first two of those are worth reading together. The daemon defect was the
*same* os-release misreading ticket 40 fixed in `install.sh`, sitting untouched
in the second place that reads the file; and the icon defect shipped inside the
very release that added the icons, because `verify-deb.sh` asserted that the
file exists and nothing asserted that a loader would recognize it. Both are the
shape of gap to look for first: a fix applied at one call site, and a check that
proves the weaker of two properties.

One thing this release still leaves for someone at a real desktop, and one
thing it closed. The icon defect was reproduced and fixed against the host's own
gdk-pixbuf, so an SVG that renders is now proven rather than assumed — but
nobody has watched GNOME draw the six icons in an applications grid after an
install. And nobody has pressed Escape on Better Launcher yet; that key path is
still untouched by any edit and uncovered by any test.
