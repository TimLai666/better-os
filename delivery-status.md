...
Ticket 46 is done on branch `ticket-46` and not merged. It is the third round of
field reports from the same Zorin 18 machine, and both halves are about a
manager that knew something and said nothing. The version now has three places
to appear — a Settings About section, the sidebar header, and
`better-manager --version`, all reading the same `CARGO_PKG_VERSION` — and the
Updates screen has an explicit 檢查更新 button that runs the existing catalog
refresh off the UI thread and then says what it found, or which list it is
showing instead when the fetch did not land. The reasoning, including why the
titlebar was left alone and why a failed refresh reports no count, is in
`docs/tickets/46-version-visibility-and-update-check.md`.

Tickets 18 through 45 are done, merged, and released, and
`docs/tickets/` holds nothing else unstarted. What is waiting is decisions rather
