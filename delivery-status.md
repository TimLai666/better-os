...
The current release is `v0.2.3`. CI run 33955547574 on merge commit `056eaad`
...
Every asset was downloaded again from the public release URL, all 32 sidecars
...
downloaded packages. It is a patch release for four defects reported from one
real Zorin 18 install, so the payload change spans every GUI package rather than
one component. `v0.2.2` before it published the same eight packages from CI run
33942768617 on merge commit `b5f6e34`, `v0.2.1` from run 33871736272 on
`8dc9c7e`, and `v0.2.0` from run 33389237001 on `96b46f1`.

A released binary still embeds the manifests as they stood before its own
release, so the built-in catalog inside `better-manager` 0.2.3 carries that
release's pre-publication placeholders. That is no longer the end of the story:
a refresh from `main` gives a 0.2.3 manager a catalog that verifies 0.2.3, and
the run recorded in M48 is that path executed against the real release rather
than described — executed against 0.2.2 rather than 0.2.3, since the mechanism
was what it proved and nothing about it changed this release.
...
Ticket 42 is done, it is released as `v0.2.3`, and no ticket is open. It was cut
from four field reports
