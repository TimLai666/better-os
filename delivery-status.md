...
The current release is `v0.2.1`. CI run 33871736272 on merge commit `8dc9c7e`
...
manifests now record checksums re-hashed from those downloaded packages. It is
a patch release: the only payload change is Better Touchpad's, which now ships
the GNOME Shell adapter extension and the resident gesture service. `v0.2.0`
before it published the same eight packages from CI run 33389237001 on merge
commit `96b46f1`.

A released binary still embeds the manifests as they stood before its own
release, so the built-in catalog inside `better-manager` 0.2.1 carries that
release's pre-publication placeholders; only the next release will ship a
catalog able to verify 0.2.1.
