...
impl MonitorWindow {
                    ...
                    .child(
                        v_flex()
                            ...
                            .children(self.gpus.iter().enumerate().map(|(index, gpu)| {
                                let selected = self.active_page == MonitorPage::Gpu
                            ...
                            .children(self.npus.iter().enumerate().map(|(index, npu)| {
                                let selected = self.active_page == MonitorPage::Npu
                            ...
                            .children(self.visible_networks().enumerate().map(|(index, interface)| {
                                let usage = interface
                                    .metadata
                                    .link_speed_mbps
                ...
                v_flex()
                    ...
                    .children(groups.into_iter().map(|group| {
                        ...
                        h_flex()
                            ...
                            .when(self.settings.app_columns.memory, |this| {
                                this.child(
                                    div().w(px(110.)).text_sm().child(linux::format_bytes(
                                        group.memory,
                            ...
                            .when(self.settings.app_columns.read_speed, |this| {
                                this.child(
                                    div().w(px(108.)).text_sm().child(linux::format_rate(
                                        group.read_speed,
                                        false,
                                        self.settings.unit_base,
                                    )),
                ...
                h_flex()
                    ...
                    .child(self.metric_card(
                        ...
                        gpu.temperature_c.map_or_else(
                            || "N/A".to_string(),
                            |value| linux::format_temperature(value, self.settings.temperature_unit),
                        ),
    ...
    fn render_column_settings(&self, cx: &mut Context<Self>) -> Div {
        ...
        v_flex()
            ...
            .child(self.section_card(
                ...
                v_flex()
                    .gap_2()
                    .child(self.app_column_switch("Memory", "app-col-memory", a.memory, AppColumn::Memory, cx))
                    .child(self.app_column_switch("CPU", "app-col-cpu", a.cpu, AppColumn::Cpu, cx))
                    .child(self.app_column_switch("Drive read speed", "app-col-read-speed", a.read_speed, AppColumn::ReadSpeed, cx))
                    .child(self.app_column_switch("Drive read total", "app-col-read-total", a.read_total, AppColumn::ReadTotal, cx))
                    .child(self.app_column_switch("Drive write speed", "app-col-write-speed", a.write_speed, AppColumn::WriteSpeed, cx))
                    .child(self.app_column_switch("Drive write total", "app-col-write-total", a.write_total, AppColumn::WriteTotal, cx))
                    .child(self.app_column_switch("GPU", "app-col-gpu", a.gpu, AppColumn::Gpu, cx))
                    .child(self.app_column_switch("GPU memory", "app-col-gpu-memory", a.gpu_memory, AppColumn::GpuMemory, cx))
                    .child(self.app_column_switch("Encoder", "app-col-encoder", a.encoder, AppColumn::Encoder, cx))
                    .child(self.app_column_switch("Decoder", "app-col-decoder", a.decoder, AppColumn::Decoder, cx))
                    .child(self.app_column_switch("Swap", "app-col-swap", a.swap, AppColumn::Swap, cx))
                    .child(self.app_column_switch("Combined memory", "app-col-combined", a.combined_memory, AppColumn::CombinedMemory, cx)),
                cx,
            ))
            .child(self.section_card(
                "Process columns",
                ...
                v_flex()
                    .gap_2()
                    .child(self.process_column_switch("PID", "proc-col-pid", p.pid, ProcessColumnSetting::Pid, cx))
                    .child(self.process_column_switch("User", "proc-col-user", p.user, ProcessColumnSetting::User, cx))
                    .child(self.process_column_switch("Memory", "proc-col-memory", p.memory, ProcessColumnSetting::Memory, cx))
                    .child(self.process_column_switch("CPU", "proc-col-cpu", p.cpu, ProcessColumnSetting::Cpu, cx))
                    .child(self.process_column_switch("Drive read speed", "proc-col-read-speed", p.read_speed, ProcessColumnSetting::ReadSpeed, cx))
                    .child(self.process_column_switch("Drive read total", "proc-col-read-total", p.read_total, ProcessColumnSetting::ReadTotal, cx))
                    .child(self.process_column_switch("Drive write speed", "proc-col-write-speed", p.write_speed, ProcessColumnSetting::WriteSpeed, cx))
                    .child(self.process_column_switch("Drive write total", "proc-col-write-total", p.write_total, ProcessColumnSetting::WriteTotal, cx))
                    .child(self.process_column_switch("GPU", "proc-col-gpu", p.gpu, ProcessColumnSetting::Gpu, cx))
                    .child(self.process_column_switch("GPU memory", "proc-col-gpu-memory", p.gpu_memory, ProcessColumnSetting::GpuMemory, cx))
                    .child(self.process_column_switch("Encoder", "proc-col-encoder", p.encoder, ProcessColumnSetting::Encoder, cx))
                    .child(self.process_column_switch("Decoder", "proc-col-decoder", p.decoder, ProcessColumnSetting::Decoder, cx))
                    .child(self.process_column_switch("Total CPU time", "proc-col-total-cpu", p.total_cpu_time, ProcessColumnSetting::TotalCpuTime, cx))
                    .child(self.process_column_switch("User CPU time", "proc-col-user-cpu", p.user_cpu_time, ProcessColumnSetting::UserCpuTime, cx))
                    .child(self.process_column_switch("System CPU time", "proc-col-system-cpu", p.system_cpu_time, ProcessColumnSetting::SystemCpuTime, cx))
                    .child(self.process_column_switch("Priority", "proc-col-priority", p.priority, ProcessColumnSetting::Priority, cx))
                    .child(self.process_column_switch("Swap", "proc-col-swap", p.swap, ProcessColumnSetting::Swap, cx))
                    .child(self.process_column_switch("Combined memory", "proc-col-combined", p.combined_memory, ProcessColumnSetting::CombinedMemory, cx))
                    .child(self.process_column_switch("Command line", "proc-col-command", p.command_line, ProcessColumnSetting::CommandLine, cx))
                    .child(self.setting_switch(
    ...
    fn signal_pids(&mut self, pids: &[Pid], signal: Signal) {
        ...
        for pid in pids {
            match self.system.process(*pid).and_then(|process| process.kill_with(signal)) {
    ...
    fn visible_disks(&self) -> impl Iterator<Item = &DiskInfo> {
        self.disk_info.iter().filter(|disk| {
            self.settings.show_virtual_drives || !disk.metadata.is_virtual
        })
    }
